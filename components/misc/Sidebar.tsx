...
export default function Sidebar(props: any) {
    ...
    return (
        <div className={`sticky`}>
            <button className={`absolute top-4 left-4 ${isSmallScreen ? "block" : "hidden"}`} onClick={toggleSidebar}>
                ☰
            </button>
            <div className={`h-[100vh] gap-2 ${isSmallScreen && !isSidebarOpen ? "hidden" : "flex"} flex flex-col bg-[#fff] transition-all duration-300 z-50 ${isSmallScreen ? "w-screen" : "w-[270px]"}`}> {/* Adjusted width for large and small screens */}
                <div className="w-full h-[70vh] flex flex-col gap-7 overflow-y-auto scrollbar-hide mr-6 mt-2">
                    {
                ...
                <div className="w-full h-36 bg-[#E8E2F4] text-lg font-medium flex flex-col justify-center items-center gap-2 mt-6 relative rounded-lg">
