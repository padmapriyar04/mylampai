...
export default function Careers() {
    ...
    const renderForm = () => {
        switch (selectedProfile) {
            case 'type3':
                return <div className="p-4">Form 1</div>;
            case 'type2':
                return <div className="p-4">Form 2</div>;
            case 'type1':
                return <div className="p-4">Form 3</div>;
            default:
                return <div className="p-4">Please select a profile to see the form.</div>;
        }
    };
            ...
            <div className="bg-primary min-h-[calc(100vh-4rem)] w-full flex justify-top text-black gap-4 p-4">
                <div className="shadow-2xl h-[calc(100vh-6rem)] max-w-[20vw] w-full rounded-xl items-center flex flex-col justify-evenly gap-[1vh]">
                    <div className="bg-white h-[500px] w-full p-6 rounded-lg">
                        <h2 className="text-2xl font-bold mb-4">Profile Selection</h2>
                        <div>
                            <button
                                className={`w-full mb-2 p-2 rounded ${selectedProfile === 'SDE' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
                                onClick={() => setSelectedProfile('type3')}
                            >
                                type1
                            </button>
                            <button
                                className={`w-full mb-2 p-2 rounded ${selectedProfile === 'AI/ML' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
                                onClick={() => setSelectedProfile('type2')}
                            >
                                type2
                            </button>
                            <button
                                className={`w-full mb-2 p-2 rounded ${selectedProfile === 'Consulting' ? 'bg-blue-500 text-white' : 'bg-gray-300'}`}
